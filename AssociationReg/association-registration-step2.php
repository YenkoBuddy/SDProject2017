...
$accesslevel ="2";
    ...
    $sql1="SELECT * FROM association WHERE AssociationName=:associationName";
    $stmt1=$conn->prepare($sql1);
    $stmt1->bindParam(':associationName', $associationName);
    $stmt1->execute(array(':associationName'=>$associationName));
    $assocInfo = $stmt1->fetch(PDO::FETCH_ASSOC);
    if(!$assocInfo){    
				$msg = "Error finding association";
			   	print_r($stmt1->errorCode());
    		...
    		else
    		{
    			extract($assocInfo);
    			session_start();
    			$_SESSION['AssociationID'] = $assocInfo['AssociationID'];
        		$associationID = $_SESSION["AssociationID"];
        		//header("refresh:5;association-registration-step1.php"); // redirects image view page after 5 seconds.
       		 }
		}
    if(isset($_POST['finish']) AND isset($associationID))
    {
...
<form method='post'>
	<table>
	<tr><td><label><?php echo $associationName; ?></label></td></tr>
	...
	<tr><td><button type="submit" name="finish" class="btn">Finish</button></td></tr>
	</table>
</form>
	    <?php if(isset($msg)){ ?>
